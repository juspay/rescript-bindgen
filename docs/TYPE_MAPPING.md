...
| open-literal idiom — `(string & {})` / `(number & {})` (the `& {}` autocomplete escape, with **no** marker prop) | `string` / `float` | semantically JUST the primitive — the `& {}` only stops TS widening the literal hints in an editor; it carries no data. Reduced at ALL depths (standalone, opaque-module union arm, record field), and both the raw intersection and its `DeepPartial`-projected apparent object (String/Number prototype). Distinct from a **branded** primitive (which has a marker prop → its own `@unboxed`) and untouched inside a **csstype** value union (already `string` via the `Globals` collapse). Prevents a bogus `{ ...JsxDOM.domProps }` bag (`hasHtml` used to misfire on String's lib.es prototype). In a MIXED union a redundant idiom arm is dropped: `string \| number \| (string & {})` → `stringOrNumber`. Fixture: [`open-string-idiom`](../test/golden/cases/open-string-idiom). (#210) |
...
| a **single** string literal — `_format: 'body'`, a brand `__brand: 'element'`, base-ui's `@as("type") type_: 'context-menu'` | a single-tag **polyvar**, `[#"body"]`. The tag's runtime value IS the bare string (compiler-verified: `{_format: #"body"}` emits `{_format: "body"}`), so this is exact and zero-cost where the previous flagged `string` both lost the value and accepted any string at all. A non-identifier value is quoted — `[#"context-menu"]`, `[#"2xl"]` — since `#context-menu` does not parse. **Enum members are excluded**: TS sets `StringLiteral` on `Size.Sm` too, and an enum member keeps its enum ref (same `EnumLike` trap the const-widening guards against — see [`enum-member-literal`](../test/golden/cases/enum-member-literal)). Also load-bearing for CORRECTNESS: as a flagged opaque the literal survived only as comment *text* while `typeSig` hashed the *kind*, so records differing solely in such a field deduped onto one entry. <br><br>**Three exclusions, each compiler-verified rather than assumed.** (a) An **all-digit** literal (`'2'`, `'0'`) falls back to the flagged `string`: ReScript compiles an all-digit tag to a JS **number** — `{v: #"2"}` emits `{v: 2}` — so a TS *string* literal would put a number on the wire silently. `'-1'`/`'1.5'` stay strings and so still narrow. (b) A **reserved word** (`'type'`, `'open'`, `'in'`, `'to'`, `'as'`) is **quoted**, not excluded: bare `[#type]` is a parse error that takes the whole file down, while `[#"type"]` compiles and emits `"type"`. (c) An **ambiguous slot of a collapsed overload set** keeps the flagged `string`. We bind only the first call signature, so any literal in a slot the overloads DISAGREE about is an artifact of that choice: narrowing a param there rejects code the library accepts (hono's `SetHeaders` first overload pins `name: 'Content-Type'`, making `c.header("X-Message", "Hello!")` inexpressible), and narrowing a return there claims a value the library will not produce (`header(~name="X-Foo")` typed `[#"set-content-type"]` while the runtime value is `"set-other"`, so a `switch` compiles away to an arm that can never match). `collapseAmbiguity` decides this **per slot** by comparing the source TEXT of each signature declaration's type nodes — syntactically, since resolving types in a gate perturbs ordering, and resolved-type *identity* is wrong for generics (each overload owns its own `T`, so hono's two `TextRespond` signatures look different while saying the same thing). `ctx.noPolyTag` then suppresses narrowing for that slot's whole SUBTREE, so a literal nested in a param's record field or inside the returned record is covered too, and it participates in the registry key — otherwise the memoized record would be built once at whichever site reached it first and the suppression would depend on visit order. Per slot and not per signature is load-bearing: hono's `c.body` overloads differ only in arity and return the identical `TypedResponse<T,U,'body'>`, which must keep `[#"body"]` or `body`/`text`/`json` re-fuse. Record-field positions and unambiguous slots are unaffected. <br><br>Fixture: [`generic-instantiation-distinct`](../test/golden/cases/generic-instantiation-distinct). (#177, #189 review) |
...
| **bare, untyped `Function`** (the global `Function` — 0 call signatures; Highcharts `proj4`, `pointDescriptionFormatter`, `complete`/`step`) | `JsFn.t` — a reference to the shared hand-authored **`JsFn`** opaque module (`type t`; `external fromFn0/1/2/3: (…args) => 'r) => t = "%identity"` + matching `asFn0/1/2/3` readers). There is no signature to type, but a `Function` IS a callable runtime leaf: the consumer wraps their own function at the matching arity (`JsFn.fromFn2(myCb)`) — honest, zero-cost, and **non-imperfect**, versus the old flagged `string` that could carry nothing. Emitted once per package (like `HtmlAttrs.res`) only when referenced. Module mode only. Fixture: [`bare-function`](../test/golden/cases/bare-function). (#120) |
| `boolean \| Config` where `Config` is a named record that *transitively contains* a bare `Function` (Highcharts `animation?: boolean \| AnimationOptionsObject`) | `@unboxed Bool(bool) \| Config(config)` — a discriminable variant (boolean vs object are distinct runtime tags). Previously the whole union collapsed to `string`: the bare `Function` inside `Config` made `Config` fail the past-`MAX_DEPTH` bounded-materialization check, so the union never re-formed. Binding `Function` to `JsFn.t` (a bounded, non-imperfect leaf) lets `Config` materialize. A `string \| Function` arm is NOT given an `@unboxed` member (its `JsFn.t` payload is abstract, so an untagged variant can't discriminate it — same limit as `React.element`, #46); such a union stays a loose `string`. Fixture: [`bool-config-union`](../test/golden/cases/bool-config-union). (#120) |
...
**Array-element unions whose arms differ ONLY in a discriminant literal collapse to one record (#177).**
The array-element branch has its own arm check (#65/#169) and bypasses the union classifier, so it had no
record-collapse fallback. It never needed one while both arms' single-literal discriminants classified to
the same flagged `string` — the arms deduped to one record and the branch never saw two. Once a lone
literal became a polyvar the arms stayed distinct, exposing the gap as the very asymmetry #181 exists to
prevent: the same union collapsed correctly as a record FIELD while degrading to opaque `%identity` views
as an ARRAY ELEMENT.

| TypeScript | ReScript |
|---|---|
| `({tag:'a';v:string} \| {tag:'b';v:string})[]` | `array<…Config>` where `type …Config = {tag: …Tag, v: string}` and `type …Tag = \| @as("a") A \| @as("b") B` — ONE directly-constructible record with a real `@as` discriminant, identical to what the same union gives in a record field |
| `(NamedA \| NamedB)[]` (named arms) | unchanged — the views module, in both positions. A collapse would fuse two library names, and following the library's own names is the rule (#62) |
| `({on:(x:string)=>void} \| {on:(x:number)=>void})[]` | unchanged — the views module. Identical key sets do NOT imply a lossless collapse: TS intersects the two `on` params to `never`, which would widen to `on: string => unit` and silently lose the second arm ("a function union is not unionable") |

The gate compares each property's **declaration text**, not its resolved type id — two arms sharing an
identical `go(): void` or inline `{n: number}` have distinct ids for identical shapes, so an id-based test
declined the collapse and reintroduced the asymmetry; its verdict effectively tracked TS's id caching.
Where the texts differ, the only losslessly mergeable case is a set of string literals (→ the `@as`
variant above). Fixture:
[`generic-instantiation-distinct`](../test/golden/cases/generic-instantiation-distinct) (`InlineArm`,
`NamedArm`, `Symmetry`, `FnArms`). (#177)
...
| the bare **`object` keyword** (TS `NonPrimitive` — "any non-primitive", no shape) | `JSON.t` — the same honest opaque-value mapping as `unknown`/`{}` (was a flagged `string`). In a **union**, its treatment depends on whether the other arms are runtime-**disjoint**: <br>• `object \| Config` — **both** arms are `typeof "object"`, so `@unboxed` can't discriminate → an **opaque module** `module …OnPoint = { type t; external fromJSON: JSON.t => t = "%identity"; external fromPlotXrangeOnPointOptions: … = "%identity"; + as* readers }` (Highcharts `onPoint`). <br>• `string \| object` (or `number \|`/`boolean \| object`) — the arms ARE runtime-disjoint (`typeof "string"` vs `"object"`), so → **`@unboxed Str(string) \| Obj(Dict.t<JSON.t>)`**. The object arm is typed `Dict.t<JSON.t>` (a *recognized* untagged-variant object shape), NOT `JSON.t` — the compiler rejects an ambiguous `JSON.t` co-payload alongside `string`. Fixture: [`object-config-union`](../test/golden/cases/object-config-union). (#120 Part B / #149) |
| `interface ArgumentArray extends Array<T> {}` (pure — no own members; classnames' shape) | `array<T>` — it IS an array. Was silently a bogus `{ ...JsxDOM.domProps }` record (Array prototype methods), losing the array shape. A HYBRID (`extends Array<T>` **plus** own fields, e.g. Highcharts' `TimeTicksInfoObject`) can't be both in ReScript, so it keeps the record of its OWN fields **only** — with **no** `...JsxDOM.domProps` spread (the Array prototype methods are lib-inherited and would misfire the spread heuristic — same guard `Map`/`Set` get, #144). `extends ReadonlyArray<T>` behaves identically (pure → `array<T>`, hybrid → own-fields record); `isArrayType` is false for it, so it's matched by symbol name. Fixture: [`coverage-papercuts`](../test/golden/cases/coverage-papercuts). (#109.2, #144) |
