...
| the bare **`object` keyword** (TS `NonPrimitive` — "any non-primitive", no shape) | `JSON.t` — the same honest opaque-value mapping as `unknown`/`{}` (was a flagged `string`). As a **union arm** it counts as a *structured* member, so `object \| Config` (Highcharts `onPoint?: object \| PlotXrangeOnPointOptions`) reaches the **opaque module** path instead of collapsing to `string`: `module …OnPoint = { type t; external fromJSON: JSON.t => t = "%identity"; external fromPlotXrangeOnPointOptions: … = "%identity"; + as* readers }` — the two arms are both `typeof "object"` so an `@unboxed` can't discriminate them, but per-arm `%identity` views can. Fixture: [`object-config-union`](../test/golden/cases/object-config-union). (#120 Part B / #149) |
