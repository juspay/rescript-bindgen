...
| `string \| ClassValue[]` where the array ELEMENT can't be modelled — incl. a **self-recursive** union (`type ClassValue = string \| number \| ClassValue[]`, clsx) | the whole union keeps the honest 🔍 REVIEW `string` flag — an **array member of an `@unboxed` is gated on its element** (mirrors the fn-return gate): an unmodellable element must not become a silent `Arr(array<string>)`. A recursive union bottoms out at `MAX_DEPTH` → `opaque`, which the gate catches, so it stays flagged instead of unrolling into a per-depth nested monster. Fixture: [`recursive-union`](../test/golden/cases/recursive-union). (#70 review) |
