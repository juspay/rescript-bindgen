...
| `string \| SVGPathArray` = `string \| Array<[Cmd] \| [Cmd, number] \| …>` (Highcharts `SVGAttributes.d`) | `@unboxed Str(string) \| Arr(array<(cmd, array<float>)>)` with a NAMED command polyvariant `type cmd = [#M \| #L \| …]` (each tag serializes to the exact letter) and the variadic numeric tail as `array<float>`. Detection requires a **string-literal head** + all-number tail, so a numeric tuple (`[number, number]`) is NOT matched. Fixture: [`svg-path-array`](../test/golden/cases/svg-path-array). (#72 rule 2) |
