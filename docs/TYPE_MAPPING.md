...
| `Record<string, V> \| Array<{ label, value: any }>` (Select's `items`) | `@unboxed type items<'a> = Dict(Dict.t<V>) \| ItemsConfigArr(array<itemsConfig<'a>>)` — a dict (object) and an array are runtime-distinct (`Array.isArray`). The element record's `value: any` is a **round-tripping generic** (you build the value, get it back via `itemToStringValue`) so it becomes `itemsConfig<'a>`, threaded to the component — **only** for array-element records (a plain record-field `any`, e.g. a state record consumed by `className`/`style`, stays flagged per #31). Fixture: [`items-dict-array`](../test/golden/cases/items-dict-array). (#50) |
