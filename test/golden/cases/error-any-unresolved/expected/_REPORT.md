...
|------|-----------------|-------------------|
| `foundationTokens` | `foundationTokens?: ThemeType;` | The declared type **does not resolve** (broken import in the package's .d.ts) → emitted as `string`. The type itself likely exists — fix the upstream import path, or hand-match it from the declaration shown. |
