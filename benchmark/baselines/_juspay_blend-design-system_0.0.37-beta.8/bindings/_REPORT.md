...
**3687** shared types deduplicated into **73** `*Types.res` modules (referenced qualified — no per-file redeclaration).
...
### Left as-is — same name, same runtime representation (140)
...
- `HighchartsSharedTypes`: `Advanced`, `All`, `Allow`, `Always`, `AnnotationMockPointOptionsObject`, `Arc`, `Area`, `Arr`, `Auto`, `Bool`, `Bottom`, `Brackets`, `C`, `Callout`, `Category`, `Chart`, `Circle`, `Close`, `Copy`, `CssObject`, `Day`, `Default`, `Diamond`, `Disabled`, `First`, `Fn`, `Full`, `High`, `Horizontal`, `Inherit` … +35 more
