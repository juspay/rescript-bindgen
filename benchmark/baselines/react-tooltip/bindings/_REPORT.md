...
|--------------------------|-----------|-------|---------------|
| `Partial<Record<"mouseenter" \| "focus" \| "mouseover" \| AnchorClickEvents, boolean>>` | `string` | 1 | openEvents |
| `Partial<Record<AnchorClickEvents \| "mouseleave" \| "blur" \| "mouseout", boolean>>` | `string` | 1 | closeEvents |
...
### Tooltip  _(also 🔍 1 to review, 2 loose)_

| Prop | Real TypeScript | Why it won't work |
|------|-----------------|-------------------|
| `middlewares` | `middlewares?: Middleware[]` | Resolved to `unknown` (generic `T` / untyped) → emitted as `string`; real values won't be used correctly. |
